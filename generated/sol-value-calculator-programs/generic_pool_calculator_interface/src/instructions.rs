...
impl GenericPoolCalculatorProgramIx {
    pub fn deserialize(buf: &[u8]) -> std::io::Result<Self> {
        ...
        match maybe_discm {
            LST_TO_SOL_IX_DISCM => {
                Ok(Self::LstToSol(LstToSolIxArgs::deserialize(&mut reader)?))
            }
            SOL_TO_LST_IX_DISCM => {
                Ok(Self::SolToLst(SolToLstIxArgs::deserialize(&mut reader)?))
            }
...
impl From<UpdateLastUpgradeSlotKeys>
for [AccountMeta; UPDATE_LAST_UPGRADE_SLOT_IX_ACCOUNTS_LEN] {
...
impl From<[Pubkey; UPDATE_LAST_UPGRADE_SLOT_IX_ACCOUNTS_LEN]>
for UpdateLastUpgradeSlotKeys {
...
impl<
    'me,
    'info,
> From<&'me [AccountInfo<'info>; UPDATE_LAST_UPGRADE_SLOT_IX_ACCOUNTS_LEN]>
for UpdateLastUpgradeSlotAccounts<'me, 'info> {
    fn from(
        arr: &'me [AccountInfo<'info>; UPDATE_LAST_UPGRADE_SLOT_IX_ACCOUNTS_LEN],
    ) -> Self {
        Self {
            manager: &arr[0],
            state: &arr[1],
...
for [AccountInfo<'info>; SET_MANAGER_IX_ACCOUNTS_LEN] {
    fn from(accounts: SetManagerAccounts<'_, 'info>) -> Self {
        [accounts.manager.clone(), accounts.new_manager.clone(), accounts.state.clone()]
    }
}
